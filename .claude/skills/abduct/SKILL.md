...
5a. **Diff-the-diff routing (the branch-point check).** When step 5 flags a *token-adjacent flip*, conjecture the hidden branch feature that separates the twins and build two oracles from it: a **base** (the foil — what the surface alone would accept) and a **reference** (the truth — what the feature-aware spec accepts). Then run a directional, second-order set-difference check (the verification analogue of OSL tri-abduction, Zilberstein, Saliling & Silva 2024, arXiv:2305.04842 — the tool only *checks*; you conjecture the branch and supply both oracles):
   ```
