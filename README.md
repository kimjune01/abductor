...
**abductor** externalizes the test so an LLM has to *represent* the rule instead of *tabulating* the example.
...
Each gate run is a perturbation; each mishandled case is a kill condition that generates the next hypothesis. The loop records itself as a **hypothesis graph** (fixes are nodes, counterexamples are edges), so the climb is legible and composable. The gate's coverage, rather than the model's prior, sets the frontier, so it drags the model past the example into the rule. In a case study on a real soundness bug, a gate of this shape took a strong model from a narrow one-case patch to a general fix that reached for the verifier's own decision procedure, where six prompt-only "reasoning method" arms stayed narrow.
...
- **You just want the fix without the trail.** If a single failing test already reproduces the bug and you don't care about generality or an audit record, fix it directly and move on.
...
<sub><em>You made it to the invariant. <a href="https://youtu.be/_Ug6gAO8n0s?si=8bK7HHL-J_5hEJbI">Look up.</a></em></sub>
